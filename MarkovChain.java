...
public class MarkovChain implements Iterator<String> {
    ...
    void addBigram(String first, String second) {
        ...
        } else if (chain.containsKey(first)) {
            this.get(first).record(second);
        } else {
            ProbabilityDistribution<String> distribution = new ProbabilityDistribution<>();
            distribution.record(second);
            chain.put(first, distribution);
        }
    }
    ...
    public void train(Iterator<String> sentence) {
        ...
        while (sentence.hasNext()) {
            ...
            if (sentence.hasNext()) {
                next = sentence.next();
                this.addBigram(curr, next);
                if (!sentence.hasNext()) {
                    this.addBigram(next, END_TOKEN);
                }
            } else {
                if (!curr.equals("")) {
                    this.addBigram(curr, END_TOKEN);
                }
            }
        }
    }
    ...
    public boolean hasNext() {
        return (this.curr != null) && !(this.curr.equals(END_TOKEN));
    }
    ...
    public String next() {
        if (!this.hasNext()) {
            ...
            String temp = this.curr;
            ProbabilityDistribution<String> distribution = this.chain.get(curr);

            if (distribution == null) {
                curr = null;
            } else {
                curr = distribution.pick(this.ng);
            }
