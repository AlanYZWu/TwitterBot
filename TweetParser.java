 ...
 * of cleaned-up words.
 * <p>
 * Note: TweetParser's methods are csvDataToTrainingData() and
...
public class TweetParser {
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        if (csvLine == null || csvLine.equals("")) {
            return null;
        }

        String[] noComma = csvLine.split(",");
        List<String> split = new LinkedList<>(Arrays.asList(noComma));

        if (csvColumn >= split.size() || csvColumn < 0) {
            return null;
        } else {
            return split.get(csvColumn);
        }
    }
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        while (iter.hasNext()) {
            ...
            if (columns > tweetColumn) {
                tweets.add(extractColumn(tweet, tweetColumn));
            }
        }
    ...
    static List<String> parseAndCleanSentence(String sentence) {
        ...
        for (String s : split) {
            if (cleanWord(s) != null && !cleanWord(s).isEmpty()) {
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        List<String> split = tweetSplit(removeURLs(tweet));
        List<List<String>> cleaned = new LinkedList<>();

        for (String s : split) {
            if (parseAndCleanSentence(s).size() > 0) {
                cleaned.add(parseAndCleanSentence(s));
            }
        }
        return cleaned;
    }
    ...
    public static List<List<String>> csvDataToTrainingData(
        ...
        for (String tweet : data) {
            ...
            if (cleaned.size() > 0) {
                for (List<String> strings : cleaned) {
                    List<String> cleanedTweet = new LinkedList<>();

                    for (int j = 0; j < strings.size(); j++) {
                        cleanedTweet.add(strings.get(j).toLowerCase());
                    }

                    training.add(cleanedTweet);
                }
            }
        }
